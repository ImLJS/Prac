...
sender_pass = 'miqbnzkcfyeeiwhr'  # enter sender's password
...
def voting():
    if request.method == 'POST':
        ...
        recognizer.read(r"C:\Users\leone\OneDrive\Desktop\Voting System Project\Trained.yml")
        cam = cv2.VideoCapture(0, cv2.CAP_DSHOW)
...
#         pkl_file.close()
#         recognizer = cv2.face.LBPHFaceRecognizer_create()
#         recognizer.read(r"E:\\College Project\\Voting Machine Project\\Smart-Voting-System-main\\code\\Trained.yml")
#         cam = cv2.VideoCapture(0, cv2.CAP_DSHOW)
#         font = cv2.FONT_HERSHEY_SIMPLEX
#         flag = 0
#         detected_persons = []
...
def voting_res():
    ...
    all_freqs = []
    for img in all_imgs:
        if img in counts['vote'].values:
            freq = counts[counts['vote'] == img].iloc[0, 1]
        else:
            freq = 0
